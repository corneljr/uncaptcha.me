(function() {
  var captcha, data, elements, initialize, utilities, view, _;
  ...
  _ = (function() {
    ...
    _.prototype.watch = function(event, callback) {
      return this.each(function(node) {
        return node.addEventListener(event, callback, false);
      });
    };

    _.prototype.unwatch = function(event, callback) {
      return this.each(function(node) {
        return node.removeEventListener(event, callback);
      });
    };

    _.prototype.get = function(attr) {
      return this.each(function(node) {
        return node.getAttribute(attr);
      });
    };

    _.prototype.set = function(attr, replacement) {
      return this.each(function(node) {
        return node.setAttribute(attr, replacement);
      });
    };
    ...
    _.prototype.addClass = function(className) {
      return this.each(function(node) {
        if (node.className.indexOf(className) === -1) {
          return node.className = ("" + node.className + " " + className).replace(/\s+/, ' ');
        }
      });
    };
    ...
    _.prototype.removeClass = function(className) {
      return this.each(function(node) {
        return node.className = node.className.replace(new RegExp(className, "gi"), '').trim();
      });
    };
  ...
  view = {
    ...
    set: function() {
      return [["state", "#state-container"], ["stateViewer", "#state-viewer"], ["image", "#image-container"], ["imageViewer", "#image-viewer"], ["response", "#response-container"], ["colors", ".captcha-button"], ["helpersContainer", "#helpers-container"], ["helpers", "#helper-text, #helper-actions"], ["action", "#helper-actions"], ["input", "#captcha_id"]].forEach(function(element) {
        return elements[element[0]] = new _([element[1]]);
      });
    },
    show: {
      captcha: function() {
        elements.image.addClass("show");
        return elements.imageViewer.set("src", utilities.base64("gif", data.captcha.image));
      },
      helpers: function(state) {
        elements.helpersContainer.addClass("show");
        return elements.helpers.each(function(node) {
          return node.innerText = node.getAttribute("data-" + state);
        });
      },
      state: function(state) {
        elements.state.addClass("show " + state);
        return elements.stateViewer.set("src", utilities.base64("png", data.image[state]));
      },
      respond: function() {
        return elements.response.addClass("show");
      }
    },
    hide: {
      captcha: function() {
        elements.image.removeClass("show");
        return elements.imageViewer.set("src", "");
      },
      helpers: function() {
        return elements.helpersContainer.removeClass("show");
      },
      state: function() {
        elements.state.removeClass("show");
        elements.state.removeClass("retry");
        elements.state.removeClass("success");
        return elements.state.removeClass("start");
      },
      respond: function() {
        return elements.response.removeClass("show");
      }
    }
  };
  ...
  captcha.reset = function() {
    ...
    view.hide.helpers();
    view.hide.respond();
    return clearTimeout(data.captcha.waiter);
  };

  captcha.start = function(state) {
    captcha.reset();
    view.show.state(state);
    return captcha.get(function() {
      return elements.state.watch("click", captcha.show);
    });
  };
  ...
  captcha.show = function() {
    ...
    return elements.action.watch("click", function() {
      return captcha.start("play");
    });
  };

  captcha.respond = function() {
    view.hide.captcha();
    view.hide.helpers();
    view.show.respond();
    view.show.helpers("respond");
    return captcha.watch();
  };

  captcha.logClicks = function() {
    var color;
    color = this.getAttribute("id").charAt(0);
  ...
  captcha.watch = function() {
    return elements.colors.watch("click", captcha.logClicks);
  };

  captcha.query = function() {
    view.hide.respond();
    view.hide.helpers();
    view.show.state("loading");
    elements.colors.unwatch("click", captcha.logClicks);
    return captcha.check(function() {
      return captcha.start(data.captcha.success ? "success" : "retry");
    });
  };

  captcha.get = function(finish) {
    var http;
    console.log("Getting...");
